    ...
    function sectionFactory (keywords, type, array) {
        ...
        switch (type) {
            ...
            case 'PAGES':
                ...
                if(INSIGHT_CONFIG.ARTICLE_LIMIT>0)  array=array.slice(0,INSIGHT_CONFIG.ARTICLE_LIMIT);
                $searchItems = array.map(function (item) {
                    ...
                    preview = preview ? preview.slice(firstOccur, firstOccur + 80) : item.text.slice(0, 80); //預覽內容 80字
                    return searchItem('file', item.title, null, null, location.origin + item.path);
                });
                if(INSIGHT_CONFIG.ARTICLE_LIMIT>0) {
                    $searchItems.push($('<div>').addClass('ins-selectable').addClass('ins-search-item').append($('<header>').append($('<i>').html("...more"))))
                }
                break;
            case 'CATEGORIES':
                $searchItems =  array.map(function (item) {
                    let page_url= item.permalink;
                    if(Boolean(CONFIG.INSIGHT)){ //內網
                        page_url= location.origin+"/"+type.toLowerCase()+"/"+item.name;
                    }
                    return searchItem(type === 'CATEGORIES' ? 'folder' : 'tag', item.name, item.slug, null, page_url);
                });
                break;
            case 'TAGS':
                //標籤取20組
                if(INSIGHT_CONFIG.TAG_LIMIT>0)  array=array.slice(0,INSIGHT_CONFIG.TAG_LIMIT);
                $searchItems =  array.map(function (item) {
                    let page_url= item.permalink;
                    if(Boolean(CONFIG.INSIGHT)){ //內網
                        page_url= location.origin+"/"+type.toLowerCase()+"/"+item.name;
                ...
                if(INSIGHT_CONFIG.TAG_LIMIT>0) { //...more
                    $searchItems.push($('<div>').addClass('ins-selectable').addClass('ins-search-item').append($('<header>').append($('<i>').html("...more"))) .attr('data-url', location.origin+"/"+type.toLowerCase()))
                }
                break;
            default:
                return null;
        ...
        return section(sectionTitle).append($('<div>').addClass('ins-seation-'+type.toLowerCase()).append($searchItems));
    }
